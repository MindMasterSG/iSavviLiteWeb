...
insert  into `deployments`(`DeploymentId`,`DeploymentName`,`Description`,`AddedDate`,`ClientName`,`Address`) values (1,'Hyd Public',NULL,'2012-12-31 00:00:00','HYD Govt','Somajiguda HYD'),(2,'KV',NULL,'2013-01-28 00:00:00','K V','K V Gachibowli'),(3,'CB',NULL,'2013-01-29 00:00:00','INFY',NULL),(4,'CA',NULL,'2013-03-18 00:00:00','CA Technologies','Hyderbad'),(5,'TPTY',NULL,'2013-03-21 00:00:00',NULL,NULL),(6,'SKS',NULL,'2013-03-21 00:00:00',NULL,NULL),(7,'SING',NULL,'2013-03-21 00:00:00',NULL,NULL);
...
insert  into `occurances`(`occuranceid`,`serialnumber`,`reportdate`,`occurancetype`,`notes`,`important`,`heading`,`deploymentid`) values (1,NULL,'2013-03-05 00:00:00',NULL,'231412343241',0,'First Occurance',NULL),(3,NULL,'2013-08-03 00:00:00',NULL,'Need to follow up',1,'GoodWork',NULL),(4,NULL,'2012-12-15 00:00:00',NULL,'Not an important one.',0,'Major tragedy',NULL),(6,NULL,'2013-03-17 00:00:00',NULL,'happening',1,'MyOccurrance',NULL),(9,NULL,'2013-03-22 16:12:54',NULL,'major work happening in the day of holi and new year holidays.',1,'Adding another occurrance',2),(10,NULL,'2013-03-24 12:39:16',NULL,'Accident',1,'Very imp occurance',2),(11,NULL,'2013-03-24 12:44:41',NULL,'sjdhfshhdfkshfkhshdfs f sdfshfhskfhshfklshklfhsklhflkshflkhsdlkfhslkhflksahflkhslkdfhskdhfklshflkshlfkhskdfhskhfklshfkshkfhsdklhfksdhflkhsdfhaslkhdflsahfkashfklshfklshdfksdhfkashflkds',1,'test',2);
...
insert  into `visitorcheckinout`(`nricid`,`name`,`address`,`fromcompany`,`tocompany`,`telno`,`persontovisit`,`eventid`,`temp`,`visitortype`,`keyid`,`passid`,`gateid`,`vehicleplatenumber`,`guardname`,`remarks`,`checkintime`,`checkouttime`,`snapshotpath`,`deploymentid`) values ('11111','sairam','asdsad','Inno',1,'8878787878','',1,0,2,0,5,3,NULL,'Guru','','2013-03-19 12:09:53','2013-03-19 12:10:55','../../Content/images/default_person.jpg',NULL),('2222','Gourav','','microsoft',1,'98778837848','',1,0,3,0,6,5,NULL,'Raj','','2013-03-22 09:08:52','2013-03-22 09:13:08','../Snapshots/20133229846297.png',NULL),('7495849','Harish','','microsoft',2,'8898328','',0,0,1,0,6,0,NULL,'','','2013-03-24 23:27:42','2013-03-24 23:28:55','../../Content/images/default_person.jpg',2),('7899930','Harish','4th street','microsoft',2,'8898328','',0,0,1,0,5,0,NULL,'','','2013-03-24 23:25:20','0001-01-01 00:00:00','../../Content/images/default_person.jpg',2);
...
BEGIN
declare result int;
SELECT COUNT(*) into result FROM `companies` WHERE `CompanyName` = p_CompanyName;
IF (result > 0) THEN
	  SET p_result = 2;
ELSE
	INSERT INTO `sms`.`companies` (`CompanyName`,`Address`,`ContactNumber`,`DeploymentId`)
...
BEGIN
DECLARE result INT;
SELECT COUNT(*) INTO result FROM `eventmasterlist` WHERE `eventname` = p_eventname;
IF (result > 0) THEN
	SET p_result = 2;
ELSE
...
BEGIN
DECLARE result INT;
SELECT COUNT(*) INTO result FROM `keys` WHERE `KeyName` = p_KeyName;
IF (result > 0) THEN
	SET p_result = 2;
ELSE
...
BEGIN
DECLARE result INT;
	SELECT COUNT(*) INTO result FROM `locations` WHERE `LocationName` = p_LocationName;
	IF (result > 0) THEN
		SET p_result = 2;
	ELSE
...
BEGIN
DECLARE result INT;
SELECT COUNT(*) INTO result FROM `passes` WHERE `PassName` = p_PassName;
IF (result > 0) THEN
	SET p_result = 2;
ELSE
...
BEGIN
DECLARE result INT;
SELECT COUNT(*) INTO result FROM `userdetails` WHERE `LoginID` = p_LoginID;
IF (result > 0) THEN
	SET p_result = 2;
ELSE
...
SELECT COUNT(*) INTO result1 FROM `visitorcheckinout` WHERE `nricid` = p_nricid AND `eventid` = p_eventid;
...
SELECT COUNT(*) INTO result1 FROM `visitorcheckinout` WHERE `nricid` = p_nricid AND DATE(checkintime) = DATE(NOW());
IF (result1 = 1) THEN SET p_result = 2;
UPDATE `visitorcheckinout` SET `checkouttime` = p_checkouttime WHERE `nricid` = p_nricid AND DATE(checkintime) = DATE(NOW());
ELSE SET p_result = 0;
END IF;
IF (p_result = 0) THEN
	INSERT INTO `visitorcheckinout`
...
BEGIN
	update `sms`.`passes` set `PassName` = p_PassName,`PassStatus` = p_PassStatus,
