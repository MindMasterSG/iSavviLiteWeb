...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`etracsms` /*!40100 DEFAULT CHARACTER SET latin1 */;
...
IF (result > 0) THEN
	  SET p_result = 2;
ELSE
	INSERT INTO `etracsms`.`deployments` (`DeploymentName`,`Description`,`AddedDate`,`ClientName`,`Address`)
...
BEGIN
	INSERT INTO `etracsms`.`incidents` (`reportdate`,`natureofincident`,`dateofincident`,
...
BEGIN
	INSERT INTO `etracsms`.`lostandfound` (`nameofperson`,`nric`,`reporteddate`,`remarks`,`deploymentid`)
...
BEGIN
	INSERT INTO `etracsms`.`occurances` (`serialnumber`,`reportdate`,`occurancetype`,`notes`,`important`,`heading`,`deploymentid`)
...
IF (result > 0) THEN
	SET p_result = 2;
ELSE
	INSERT INTO `etracsms`.`passes` (`PassName`,`PassStatus`,`VisitortypeId`,`DeploymentId`)
...
IF (result > 0) THEN
	delete from `etracsms`.`companies` WHERE `CompanyId` = p_CompanyId;
	SET p_result = 1;
ELSE
	SET p_result = 2;
END IF;
...
BEGIN
	delete from `etracsms`.`incidents` 	where `incidentid` = p_IncidentID;
END */;
...
BEGIN
	delete from `etracsms`.`lostandfound` 
...
BEGIN
	delete from `etracsms`.`occurances` 	where `occuranceid` = p_occuranceid;
END */;
...
BEGIN
	delete from `etracsms`.`passes` where `PassId` = p_PassId;
END */;
...
BEGIN
	select `CompanyId`,`CompanyName`,`Address`,`ContactNumber`,`DeploymentId` from `etracsms`.`companies`;
END */;
...
BEGIN
	select `CompanyId`,`CompanyName`,`Address`,`ContactNumber`,`DeploymentId` from `etracsms`.`companies` WHERE `CompanyId` = p_CompanyId;
END */;
...
BEGIN
	select `lostandfoundreportid`,`nameofperson`,`nric`,`reporteddate`,`remarks`,`deploymentid` from `etracsms`.`lostandfound` 
	where `lostandfoundreportid` = p_lostandfoundreportid;
END */;
...
BEGIN
	select `PassId`,`PassName`,`PassStatus`,`VisitortypeId`,`DeploymentId` from `etracsms`.`passes`;
END */;
...
/*!50003 CREATE PROCEDURE `UpdateIncident`(in p_incidentid int, IN p_reportdate datetime, IN p_natureofincident VARCHAR(20), in p_dateofincident datetime,
 ...
 in p_remarksandactionstaken varchar(500),IN p_snapshotpath VARCHAR(100), IN  p_deploymentid INT)
BEGIN
	update `etracsms`.`incidents` set `reportdate` = p_reportdate,`natureofincident` = p_natureofincident,
...
in p_reporteddate datetime, in p_remarks varchar(500), IN  p_deploymentid INT)
BEGIN
	update `etracsms`.`lostandfound` set `nameofperson` = p_nameofperson,`nric` = p_nric,
...
BEGIN
	update `etracsms`.`occurances` set `serialnumber` = p_serialnumber,`reportdate` = p_reportdate,
